...
are included in the total order \tcode{S} on \tcode{memory_order::seq_cst} operations). A heavyweight-fence
with acquire semantics is called an acquire heavyweight-fence. A heavyweight-fence has all the
synchronization effects of a fence (\CppXref{33.5.11} [atomics.fences]). \begin{note}  Heavyweight-fences and
