...
\newenvironment{#1}
{\begin{tail#1}}
{\end{tail#1}\small\par}   % \small\par is for ISO/IEC 14882:2020 post-DIS compatibility
}

\newnoteenvironment{note}{NOTE \arabic{note}}{end note}

% \newexplenvironment{ENVIRON}{BEGIN TEXT}{END TEXT}
% Creates an example-like environment beginning with BEGIN TEXT and
% ending with END TEXT. A counter with name ENVIRON indicates the
% number of this kind of note / example that has occurred in this
% subclause.
% Use tailENVIRON to avoid inserting a \par at the end.
...
\newexplenvironment{example}{EXAMPLE \arabic{example}}{end EXAMPLE}
