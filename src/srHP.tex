...
\pnum
Concurrent access to a domain does not incur a data race (\tcode{[intro.races]}).
...
\pnum
\effects 
Move-assigns \tcode{d} to \tcode{deleter}, thereby setting it as the deleter of \tcode{x}, then retires \tcode{x}
to \tcode{domain}.
