...
\pnum
An object x of hazard-protectable type T is \defn{retired} to a domain with a deleter of type \tcode{D} when the member function \tcode{hazard_pointer_obj_base<T,D>::retire} is invoked on x. Any given object x shall be retired at most once.
...
\begin{itemize}
\item x is not reclaimed, and
\item x is retired to $dom$ in an evaluation that happens before $A$, and
\item for all hazard pointers $h$ that belong to $dom$, the end of any protection epoch where $h$ is associated with x happens before $A$.
\end{itemize}
...
\begin{itemize}
\item x is not reclaimed; and
\item x is retired to $dom$ in an evaluation $R$ and $A$ does not happen before $R$; and
...
\begin{itemize}
  ...
  \item if the beginning of $E$ happens before x is retired, the end of $E$ strongly
happens before $A$, and
  \item if $E$ began by an evaluation of \tcode{try_protect} with argument \tcode{src}, label its
atomic load operation $L$. If there exists an atomic modification $B$ on \tcode{src} such that $L$ observes a modification that is modification-ordered before $B$, and $B$ happens before x is retired, the end of $E$ strongly happens before $A$. \begin{note} In typical use, a store to \tcode{src} sequenced before retiring x will be such an atomic operation $B$. \end{note}
\end{itemize}
\begin{note} The latter two conditions convey the informal notion that a protection epoch that began before retiring x, as implied either by the happens-before relation or the coherence order of some source, delays the reclamation of x. \end{note}
\end{itemize}
...
\pnum
\defn{Preconditions}: 
\tcode{*this} is a base class subobject of an object x of type T. x is not retired.
...
\pnum
\effects 
Move-assigns \tcode{d} to \tcode{deleter}, thereby setting it as the deleter of x, then retires x
to domain.
