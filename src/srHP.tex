...
\rSec1[Concurrency2.SafeRecl.HP]{Hazard pointers}

\rSec2[Concurrency2.SafeRecl.HP.general]{General}
...
\pnum
An object of type \tcode{hazard_pointer} is either empty or \defn{owns} a hazard pointer. Each hazard pointer is owned by exactly one object of type \tcode{hazard_pointer}. [ \defn{Note 1}: An empty \tcode{hazard_pointer} object is different from a \tcode{hazard_pointer} object that owns an unassociated hazard pointer. An empty tcode{hazard_pointer} object does not own any hazard pointers. --- \defn{end note} ]
...
atomic load operation L. If there exists an atomic modification B on \tcode{src} such that L observes a modification that is modification-ordered before B, and B happens before \tcode{x} is retired, the end of E strongly happens before A. [ \defn{Note 2}: In typical use, a store to \tcode{src} sequenced before retiring \tcode{x} will be such an atomic operation B. --- \defn{end note} ]
\end{itemize}
[ \defn{Note 3}: The latter two conditions convey the informal notion that a protection epoch that began before retiring \tcode{x}, as implied either by the happens-before relation or the coherence order of some source, delays the reclamation of \tcode{x}. --- \defn{end note} ]
\end{itemize}

[ \defn{Example 1}: The following example shows how hazard pointers allow updates to be carried out in the presence of concurrent readers. The object of type \tcode{hazard_pointer} in \tcode{print_name} protects the object \tcode{*ptr} from being reclaimed by \tcode{ptr->retire} until the end of the protection epoch.
...
\rSec2[Concurrency2.SafeRecl.HP.syn]{Header \tcode{<hazard_pointer>} synopsis}
...
\rSec2[Concurrency2.SafeRecl.HP.domain]{Class \tcode{hazard_pointer_domain}}

\rSec3[Concurrency2.SafeRecl.HP.domain.general]{General}

\pnum
The number of unreclaimed possibly-reclaimable objects retired to a domain is bounded.
The bound is implementation-defined. [ \defn{Note 1}: The bound can be independent of other
...
\rSec3[Concurrency2.SafeRecl.HP.domain.mem]{Member functions}

\tcode{hazard_pointer_domain() noexcept;}

\pnum
\defn{Effects}: Equivalent to \tcode{hazard_pointer_domain(\{\})}.
\\
...
\rSec2[Concurrency2.SafeRecl.HP.domain.default]{Default \tcode{hazard_pointer_domain}}
...
\rSec2[Concurrency2.SafeRecl.HP.cleanup]{Clean up}
...
\rSec2[Concurrency2.SafeRecl.HP.base]{Class template \tcode{hazard_pointer_obj_base}}
...
\pnum
\defn{Effects}: Equivalent to \tcode{retire(D(), domain)}.
\\

\rSec2[Concurrency2.SafeRecl.HP.holder]{Class \tcode{hazard_pointer}}

\rSec3[Concurrency2.SafeRecl.HP.holder.syn]{Synopsis}
...
\rSec3[Concurrency2.SafeRecl.HP.holder.ctor]{Constructors}
...
\rSec3[Concurrency2.SafeRecl.HP.holder.dtor]{Destructor}
...
\rSec3[Concurrency2.SafeRecl.HP.holder.assign]{Assignment}
...
\rSec3[Concurrency2.SafeRecl.HP.holder.mem]{Member functions}
...
\pnum
\defn{Returns}: \tcode{old == ptr}. [ \defn{Note 1}: It is possible for \tcode{try_protect} to return true when \tcode{ptr} is a null pointer. --— \defn{end note} ]
...
\pnum
\defn{Effects}: Swaps the hazard pointer ownership of this object with that of other. [ \defn{Note 2}: The owned hazard pointers, if any, remain unchanged during the swap and continue to be associated with the respective objects that they were protecting before the swap, if any. No protection epochs are ended or initiated. — \defn{end note} ]

\pnum
\defn{Complexity}: Constant.
\\

\rSec2[Concurrency2.SafeRecl.HP.make]{\tcode{make_hazard_pointer}}
...
\rSec2[Concurrency2.SafeRecl.HP.special]{\tcode{hazard_pointer} specialized algorithms}
