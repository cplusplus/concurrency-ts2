...
\rSec1[Concurrency2.SafeRecl.rcu]{Read-copy update (RCU)}

\rSec2[Concurrency2.SafeRecl.rcu.general]{RCU general}
...
\rSec2[Concurrency2.SafeRecl.rcu.syn]{RCU header \tcode{<rcu>} synopsis}
...
\rSec2[Concurrency2.SafeRecl.rcu.base]{Class \tcode{rcu_obj_base}}
...
\pnum
Preconditions: \tcode{*this} is a base class subobject of
...
\pnum	Effects: Evaluates \tcode{deleter = std::move(d)} and schedules
...
\rSec2[Concurrency2.SafeRecl.rcu.domain]{Class \tcode{rcu_domain}}
...
\rSec3[Concurrency2.SafeRecl.rcu.domain.lock]{\tcode{rcu_domain::lock}}
...
\rSec3[Concurrency2.SafeRecl.rcu.domain.unlock]{\tcode{rcu_domain::unlock}}
...
\pnum
Preconditions: A call to the function \tcode{lock} that opened
...
\rSec2[Concurrency2.SafeRecl.rcu.default.domain]{\tcode{rcu_default_domain}}
...
\rSec2[Concurrency2.SafeRecl.rcu.synchronize]{\tcode{rcu_synchronize}}
...
\pnum
Effects: If the call to \tcode{rcu_synchronize} does not strongly
happen before the lock opening an RCU protection region \tcode{R}
on \tcode{dom}, blocks until the \tcode{unlock} closing \tcode{R}
happens.

\pnum
Synchronization: The \tcode{unlock} closing \tcode{R} strongly
happens before the return from \tcode{rcu_synchronize}.

\rSec2[Concurrency2.SafeRecl.rcu.barrier]{\tcode{rcu_barrier}}
...
\pnum
Synchronization: The evaluation of any such \tcode{E} strongly
happens before the return from \tcode{rcu_barrier}.

\rSec2[Concurrency2.SafeRecl.rcu.retire]{Template \tcode{rcu_retire}}
...
\pnum
Mandates: \tcode{is_move_constructible_v<D>} is true.

\pnum
Preconditions: \tcode{D} meets the Cpp17MoveConstructible and
